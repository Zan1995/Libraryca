...
namespace Team08B_Library
{
    public partial class frmMemberUpdates : Form
    {
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (r == DialogResult.Yes) {
                using (TransactionScope ts = new TransactionScope()) {
                    ...
                    member.PlaceOfBirth = cbbCountryCode.Text.ToUpper();
        ...
        private void checkChanges()
        {
            ...
            tbxMemberName.Text != member.MemberName ||
            tbxPhone.Text != member.PhoneNumber ||
            tbxPostalCode.Text != member.PostalCode) && ( lblAddressValid.Text=="" && lblCityValid.Text=="" && lblCountryCodeValid.Text=="" && lblEmailValid.Text == "" && lblMemberNameValid.Text=="" && lblPhoneValid.Text=="" && lblPostalCodeValid.Text=="") 
            //&&
            //(tbxAddress.Text != "" &&
            //tbxCity.Text != "" &&
        ...
        {
            Regex rxCountryCode = new Regex(@"^[A-Za-z]{3}$");

            if (!rxCountryCode.IsMatch(cbbCountryCode.Text)) {
                lblCountryCodeValid.Text = "You should enter a valid country code.";
                checkChanges();
            }
            else {
                lblCountryCodeValid.Text = "";
            }
            checkChanges();
